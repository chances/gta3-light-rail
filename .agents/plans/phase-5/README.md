...
Backing up the original camera files was deferred during Phase 2. Complete this before any camera data is authored or
modified.

- [ ] Back up `train.dat` and `train2.dat` from `GTA3/data/paths/` using `scripts/backup-tracks.ts` (or a dedicated
      script if needed).
- [ ] Confirm whether the engine supports a third `train*.dat` file (same question as `tracks3.dat` — likely the same
      answer).
